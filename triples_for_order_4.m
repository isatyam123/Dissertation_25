function generate_pentadiagonal_dsm_order4()
    ...
    for n = 1:4
        ...
        theta = linspace(0, 2*pi, n+1); % Angles for roots of unity
    ...
    perms_list = generate_valid_permutation_matrices(4); % 14 valid P-matrices
    triplets = nchoosek(1:length(perms_list), 3); % 2184 valid (P1, P2, P3) triplets
    num_combinations = 10; % 10 convex combinations per triplet
    ...
    % Print all 14 valid permutation matrices
    fprintf('\nValid Permutation Matrices for Order 4 Pentadiagonal DSM:\n');
...
function perms_list = generate_valid_permutation_matrices(n)
    ...
    for i = 1:size(P_all, 1)
        ...
        % Exclude matrices with 1 at (1,4) and (4,1)
        if P(1,4) == 0 && P(4,1) == 0
