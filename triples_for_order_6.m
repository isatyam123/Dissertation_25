function generate_pentadiagonal_dsm_order6()
    % Step 1: Generate and plot convex hull structures for n = 1, 2, 3, 4, 5, 6 (in red)
    figure;
    hold on;
    ...
    perms_list = generate_valid_permutation_matrices(6); % Filtered valid P-matrices
    ...
    axis equal;
    grid on;
    title('Convex Hull Structures (Red) and Eigenvalues (Blue) for Order 6 Pentadiagonal DSM');
    xlabel('Real Part');
    ylabel('Imaginary Part');
    hold off;

    % Print all valid permutation matrices
    fprintf('\nValid Permutation Matrices for Order 6 Pentadiagonal DSM:\n');
...
function perms_list = generate_valid_permutation_matrices(n)
    ...
    for i = 1:size(P_all, 1)
        ...
        % Exclude matrices with 1 at restricted positions
        if P(1,4) == 0 && P(1,5) == 0 && P(1,6) == 0 && ...
           P(2,5) == 0 && P(2,6) == 0 && P(3,6) == 0 && ...
           P(4,1) == 0 && P(5,1) == 0 && P(5,2) == 0 && ...
           P(6,1) == 0 && P(6,2) == 0 && P(6,3) == 0
